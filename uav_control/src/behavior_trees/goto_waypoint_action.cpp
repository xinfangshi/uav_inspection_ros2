...
GoToWaypointAction::GoToWaypointAction(const std::string& name, const BT::NodeConfig& config,
...
BT::NodeStatus GoToWaypointAction::onStart() {
    ...
    double distance = std::sqrt(std::pow(end_pt.x - start_pt.x, 2) + std::pow(end_pt.y - start_pt.y, 2) +
                                std::pow(end_pt.z - start_pt.z, 2));
    double T = std::max(2.0, distance / 2.0);  // 最少飞 2 秒，保证起降平稳

    RCLCPP_INFO(node_->get_logger(), "[GoToWaypoint] 🛸 开始生成 Minimum Snap 轨迹! 距离: %.2fm, 预计耗时: %.1fs",
                distance, T);
...
BT::NodeStatus GoToWaypointAction::onRunning() {
    ...
    double distance = std::sqrt(std::pow(current_x_ - target_x, 2) + std::pow(current_y_ - target_y, 2) +
                                std::pow(current_z_ - target_z, 2));
...
void GoToWaypointAction::onHalted() { RCLCPP_WARN(node_->get_logger(), "[GoToWaypoint] ⚠️ 导航被强行中断！"); }
